		...
		add_filter( 'colibriwp_theme_theme_plugins', function ( $plugins ) {
			...
			if ( $this->isPRO() || file_exists( WP_PLUGIN_DIR . "/colibri-page-builder-pro/colibri-page-builder-pro.php" ) ) {
				$plugins['colibri-page-builder-pro'] = array(
					...
					'description' => $plugins['colibri-page-builder']['description'],
					'priority'    => 0
				);
...
//                }
	...
	public function addEditInCustomizerPageButtons() {
		...
		if ( $this->canEditInCustomizer( $post ) ) {
			echo '<a href="javascript:void();"  onclick="cp_open_page_in_customizer(' . $post->ID . ')"  class="button button-primary">' . __( 'Edit In Customizer',
            ...
            <script type="text/html" id="colibri-page-builder-guttenberg">
                <a href="javascript:void();"
                   ...
                   class="button button-primary button-hero" style="height:40px; line-height: 40px">
                    ...
                    <span style=" text-shadow: none; font-size: 14px;"><?php _e( 'Edit In Customizer',
                ...
                function cp_open_page_in_customizer(page) {
                    ...
                    if (isAutodraft) {
                        ...
                        if (wp.data) {
                            ...
                            if (!documentTitle) {
                                wp.data.dispatch('core/editor').editPost({title: 'Colibri #' + $('#post_ID').val()});
                            }
